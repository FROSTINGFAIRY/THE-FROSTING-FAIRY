...
export default function RecipeDetail({
  ...
  const boxCapacity = recipe.boxCapacity || 6;
  const [boxSelections, setBoxSelections] = useState<Record<string, number>>({});

  // Eligible products for box building
  const eligibleProducts = React.useMemo(() => {
    if (!isBox) return [];
    return allRecipes.filter((r) => {
      if (r.id === recipe.id || r.isBuildYourBox) return false;
      if (recipe.boxEligibleCategories && recipe.boxEligibleCategories.length > 0) {
        return recipe.boxEligibleCategories.some(
          (cat) => cat.toLowerCase() === r.category.toLowerCase()
        );
      }
      return true;
    });
  }, [allRecipes, isBox, recipe.id, recipe.isBuildYourBox, recipe.boxEligibleCategories]);
  ...
  const handleBoxItemChange = (productName: string, delta: number) => {
    setBoxSelections((prev) => {
      ...
      const otherTotal = (Object.entries(prev) as [string, number][])
        .filter(([name]) => name !== productName)
        .reduce((sum: number, [_, q]) => sum + (q || 0), 0);
      
      if (delta > 0 && otherTotal + next > boxCapacity) {
        return prev;
      }
  ...
  const handleAddToCartClick = () => {
    if (isBox && totalBoxItemsSelected !== boxCapacity) {
      return;
    }
    ...
    onAddToCart({
      ...
      selectedOption: isBox ? `Assorted Box (${boxCapacity} Pieces)` : priceOption.label,
      price: priceOption.price,
      amount: quantity,
      unit: 'Qty',
      image: customImageUrl || recipe.image,
      customMessage: recipe.category === 'Signature Cakes' ? customMessage : isBox ? customMessage : '',
      recipeName: isBox ? `${boxCapacity}-Item Custom Assortment` : frostingFlavor,
      boxContents,
    });
          ...
          <div className="bg-white border border-brand-cocoa-border rounded-2xl p-6 shadow-md text-left space-y-5">
            ...
            {isBox ? (
              ...
              <div className="space-y-5">
                {/* Box Capacity & Live Fill Progress */}
                <div className="bg-brand-pink-light/20 border border-brand-pink/30 rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono font-bold uppercase tracking-wider text-brand-cocoa">
                      Box Fill Status
                    </span>
                    <span className="text-xs font-mono font-extrabold text-brand-pink bg-white px-2.5 py-1 rounded-full border border-brand-pink/30 shadow-3xs">
                      {totalBoxItemsSelected} / {boxCapacity} Items
                    </span>
                  </div>

                  {/* Visual Progress Bar */}
                  <div className="w-full bg-brand-cream-light border border-brand-cocoa-border/40 h-3 rounded-full overflow-hidden p-0.5">
                    <div
                      className="bg-brand-pink h-full rounded-full transition-all duration-300"
                      style={{ width: `${Math.min(100, (totalBoxItemsSelected / boxCapacity) * 100)}%` }}
                    />
                  </div>

                  {/* Status Banner */}
                  <div className="flex items-center gap-2 pt-1">
                    {totalBoxItemsSelected < boxCapacity ? (
                      <p className="text-xs font-sans font-medium text-brand-cocoa-light flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                        <span>
                          Pick <strong>{boxCapacity - totalBoxItemsSelected} more</strong> {boxCapacity - totalBoxItemsSelected === 1 ? 'treat' : 'treats'} to complete this box.
                        </span>
                      </p>
                    ) : (
                      <p className="text-xs font-sans font-bold text-emerald-700 flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl w-full">
                        <Check className="w-4 h-4 text-emerald-600" />
                        <span>Box is fully packed and ready for order!</span>
                      </p>
                    )}
                  </div>
                </div>

                {/* Eligible treats selection list */}
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold font-mono uppercase tracking-wider text-brand-cocoa-light">
                      Select Your Favorite Pastries & Confections
                    </label>
                    <span className="text-[10px] font-mono text-brand-cocoa-light/80">
                      Capacity: {boxCapacity} pieces
                    </span>
                  </div>

                  <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                    {eligibleProducts.length === 0 ? (
                      <p className="text-xs text-brand-cocoa-light italic py-4 text-center">
                        No individual items currently available for assortment.
                      </p>
                    ) : (
                      eligibleProducts.map((item) => {
                        const count = boxSelections[item.name] || 0;
                        const isSelected = count > 0;
                        const isMaxReached = totalBoxItemsSelected >= boxCapacity;
                        ...
                        return (
                          <div
                            ...
                            className={`p-3 rounded-xl border transition-all flex items-center justify-between gap-3 ${
                              isSelected
                                ? 'bg-brand-pink-light/20 border-brand-pink/50 shadow-3xs'
                                : 'bg-white border-brand-cocoa-border hover:border-brand-pink-accent/40'
                            }`}
                          >
                            ...
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              ...
                              <div className="min-w-0 flex-1 text-left">
                                ...
                                <span className="inline-block text-[9px] font-mono font-semibold text-brand-cocoa-light bg-brand-cream px-1.5 py-0.5 rounded mt-0.5">
                                  {item.category}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <button
                                ...
                                className={`w-7 h-7 rounded-lg border flex items-center justify-center transition-all cursor-pointer ${
                                  count === 0
                                    ? 'border-brand-cocoa-border/40 text-brand-cocoa-light/40 bg-brand-cream cursor-not-allowed'
                                    : 'border-brand-cocoa-border bg-white hover:bg-brand-pink-light text-brand-cocoa shadow-3xs'
                                }`}
                              >
                              ...
                              <button
                                ...
                                className={`w-7 h-7 rounded-lg border flex items-center justify-center transition-all cursor-pointer ${
                                  isMaxReached
                                    ? 'border-brand-cocoa-border/40 text-brand-cocoa-light/40 bg-brand-cream cursor-not-allowed'
                                    : 'border-brand-pink bg-brand-pink text-white hover:bg-brand-pink-dark shadow-3xs'
                                }`}
                              >
                ...
                <div className="space-y-2 pt-2 border-t border-brand-cocoa-border/40">
                  ...
                  <input
                    type="text"
                    maxLength={60}
                    placeholder="e.g. For my favorite sweet tooth! Happy Anniversary"
                    value={customMessage}
            ...
            <div className="border-t border-brand-cocoa-border/50 pt-4 flex flex-col gap-2">
              ...
              <button
                type="button"
                disabled={isBox && totalBoxItemsSelected !== boxCapacity}
                ...
                className={`w-full font-sans font-bold py-4 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 group cursor-pointer ${
                  isBox && totalBoxItemsSelected !== boxCapacity
              ...
              >
                ...
                <span>
                  {isBox
                    ? totalBoxItemsSelected < boxCapacity
                      ? `Pick ${boxCapacity - totalBoxItemsSelected} More to Add to Cart (${totalBoxItemsSelected}/${boxCapacity})`
                      : 'Add Assorted Box to Cart 🛒'
